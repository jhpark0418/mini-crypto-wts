...
또한 TimescaleDB와 Redis를 함께 사용하여 **히스토리 캔들 / 진행 중인 Active Candle / Orderbook Snapshot**을 분리 관리하며,  
...
GET /api/candles
↓
Historical candles from TimescaleDB
↓
setData()
```
...
│   │   └─ TimescaleDB candle persistence
│   │
│   ├─ api-gateway
...
캔들 데이터는 시간 축을 중심으로 누적되는 전형적인 시계열 데이터입니다.

```
시계열 데이터에 적합한 hypertable 구조
시간 기반 chunk 분할
최근 데이터 조회 패턴에 유리한 저장 구조
보관 정책(retention), 압축(compression) 등 운영 확장성 확보
PostgreSQL 생태계를 유지하면서 시계열 최적화 가능
```
...
TimescaleDB로 전환하는 과정에서 **기존 Primary Key 설계와 hypertable 제약 조건이 충돌하는 문제**를 경험했고, 이를 해결했습니다.

#### 문제

기존 `candles` 테이블을 TimescaleDB hypertable로 전환한 뒤 검증 쿼리를 실행했을 때,  
확장(extension)은 정상 설치되었지만 `candles` 테이블이 hypertable로 등록되지 않았습니다.
...
즉, 테이블은 생성되었지만 일반 PostgreSQL 테이블로만 남아 있고,
TimescaleDB의 핵심 기능인 chunk 분할과 시계열 최적화를 전혀 사용하지 못하는 상태였습니다.

#### 원인

초기 스키마에서 candles 테이블은 다음과 같이 설계되어 있었습니다.

id BIGSERIAL PRIMARY KEY
UNIQUE(symbol, timeframe, open_time)

하지만 TimescaleDB hypertable은 Primary Key / Unique Index가 반드시 파티션 기준 컬럼을 포함해야 하는 제약이 있습니다.
이 프로젝트에서 시계열 파티션 기준 컬럼은 open_time 이었는데, 기존 PK는 id 단독 컬럼으로 구성되어 있었습니다.
...
이 구조 때문에 create_hypertable('candles', 'open_time', ...) 단계에서 전환이 실패했고,
결과적으로 candles 테이블은 hypertable로 등록되지 않았습니다.

#### 해결

1. 단일 PK(id) 제거

TimescaleDB 제약과 충돌하는 id BIGSERIAL PRIMARY KEY 구조를 제거했습니다.

2. 복합 Primary Key로 변경

캔들 데이터의 자연 키인 아래 조합을 기준으로 복합 PK를 설정했습니다.
...
한 캔들을 식별하는 비즈니스 키와 일치하고
TimescaleDB가 요구하는 open_time 포함 조건도 만족합니다.

3. 시간 컬럼을 TIMESTAMPTZ로 정리

기존 timestamp 대신 TIMESTAMPTZ를 사용하도록 수정하여
Binance 시각, REST history, 실시간 차트 데이터의 UTC 기준을 일관되게 맞췄습니다.

4. DB 초기화 방식 분리

TypeORM의 synchronize: true를 제거하고,
테이블 생성 및 hypertable 변환 책임을 SQL 초기화 스크립트로 옮겼습니다.
...
쿼리에서도 chunk pruning과 인덱스 스캔이 정상적으로 동작하는 것을 확인했습니다.
