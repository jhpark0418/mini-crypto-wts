...
또한 일부 상태 데이터는 Redis를 활용하여 관리하며, 차트 초기화 시 REST + WebSocket을 결합하여 빠른 UX를 제공합니다.
...
Redis 기반 Active Candle Snapshot
...
C -->|consume events| F[api-gateway]

F -->|WebSocket| H[React Chart]
...
- 멀티 타임프레임 캔들 생성
- CandleOpened / CandleClosed 이벤트 발행
...
또한 현재 진행 중인 캔들을 Redis에 저장하고 Active Candle Snapshot API를 제공합니다.
